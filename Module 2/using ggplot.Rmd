...
```
```{r}
ggplot(data= state_data, mapping= aes(x=Positive))  + geom_histogram(bins = 10)
```
```{r}
```
...
```{r}
ggplot(data= state_data, mapping= aes(x=Positive))  + geom_histogram(bins = 10,color= "white")
```
```{r}
ggplot(data= state_data, mapping= aes(x=Positive))  + geom_histogram(bins = 10, color = "white", fill="pink")
```
```{r}
...
```
```{r}
ggplot(data= manystates_data, mapping= aes(x=Negative)) + geom_histogram(bins = 10, color = "white", fill = "pink") + facet_wrap(~State, nrow = 2)
```
```{r}
data(mpg)
?mpg
